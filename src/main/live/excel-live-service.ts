  ...
  async readRange(range: string, sheetName?: string): Promise<{ workbook: WorkbookSummary; attachment: ChatAttachment }> {
    ...
    return {
      ...
      attachment: {
        kind: "table",
        title: result.title,
        rows: normalizeLiveRows(result.rows),
      },
    }
  }
...
function normalizeLiveRows(value: unknown): string[][] {
  if (!Array.isArray(value)) {
    return [[stringifyLiveCell(value)]]
  }
  ...
  if (value.every((row) => Array.isArray(row))) {
    return value.map((row) => row.map((cell) => stringifyLiveCell(cell)))
  }

  return [value.map((cell) => stringifyLiveCell(cell))]
}
